...
public class UnbanCommand extends BaseCommand {
    ...
    protected boolean execute(CommandSender sender, String[] args) {
        ...
        if (!punishmentService.isBanned(targetUUID)) {
            sendMessage(sender, MessagesConfig.get("punishments.unban.not-banned").replace("%target%", targetName));
            return true;
        }
        ...
        // Execute the unban
        punishmentService.unban(issuerUUID, targetUUID);
    ...
    protected List<String> tabComplete(CommandSender sender, String[] args) {
        ...
        if (args.length == 1) {
            // Tab complete banned player names
            String partialName = args[0].toLowerCase();
            
            // This is a simplified tab completion - in a real implementation,
            // you might want to query the database for banned players
